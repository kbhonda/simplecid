...
とすれば，丸数字はそれぞれの書体でそのときのサイズのものが出力されます．ただし，

多書体化の場合はpLaTeX2eでの和文の扱いの知識が必要ですが，使用したい書体に対してNFSSでどのような名前をつけているのか（`\DeclareFontShape`でどのように宣言しているのか）と，実際のフォントのファイル名が分かればいいだけです．それが分からない場合でも，`\sCID`を使った際に適切な設定がなされていないときにはそれなりのエラーがでるので，それを利用すればなんとかなります．

## TODOを兼ねた言い訳

上記でも触れていることもありますが，若干の言い訳らしきものを．

NFSSでの名前と実フォントは1対1ではありません．また，`\DeclareFontShape`で`ssub`や`sub`を使った代替まで探査するのはあまりにつらい（できるのかも分かりません）ので，間にIDを経由させて手動で対応をつけるようにしました．IDは任意の文字列にするほうが自然だとは思いますが，tfmの名前を決め打ちにせざるを得ないのでIDはtfmの名前と一致させました．IDとtfmの名前との対応表を作ればいいのですが，本質的ではない上にそれほど利益がないのに`simplecid,sty`のコードが煩雑になるのでそれは避けました．00～09までのIDも本来なら0～9の方が自然なのかもしれませんが，min/gothという特別なIDを使うことにしたので`\two@digits`を無条件に適用できなくなり，tfmのファイル名に直接使えるものにしました．

このパッケージを使う上で，IDと実フォント，NFSS名とIDの対応がもっとも重要なので，可読性を少しでもあげるためにも，`\sCIDSetting`では他の言語の連想配列のように`=`だけでなく`=>`や`->`を許容しています．実際は単なる正規表現による置換であってsyntax sugarにすぎませんが，表のような形で整理できるので混乱はさけられるでしょう．
...
与えられたCID番号cidに対して，cid -1 = n * 8836 + R (0<=R<8836)と割り算することで，nが対応するtfmの番号となります．kこのRを94で割ることで，R = (ku-1) * 94 + (ten -1) と表すことで，cidに対する区kuと点tenを割り出します．このkuとtenをJISコードjisにするには，区点からの変換方法に従ってjis=(ku+0x20) * 0x100 + (ten+0x20)とすればよいです．

以上より，与えられたCID番号に，tfm「n」，のJISコード「jis」の文字が対応付けられます．`simplecid.sty`ではこの計算とtfmへの割り振りを行っています．

そしてtfm「n」に対応するvfでは，そのスロットの先頭からnの値に応じて次の表のCID番号を順番に対応付けます．vfから参照されるtfmにはIdentity-Hのcmapで実CIDフォントを割りあてればvfで与えられるCID番号の文字が最終的に出力されます．
