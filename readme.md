...
IDと実フォント，NFSS名とIDの対応がこのパッケージを使ううえでもっとも重要なところですので，可読性を少しでもあげるためにも，他の言語の連想配列のように`=`だけでなく`=>`や`->`を許容しています．実際は単なる正規表現での置換でsyntax sugarにすぎませんが，表のような形で整理できるので混乱はさけられるでしょう．

縦組みに関しては，需要が少ないのと，文字や記号を出すだけの本パッケージではtfmまで作らなくても誤魔化せるかなという思いはあります（回転させてベースラインを調整すればいい？）．upLaTeX2eに関しては，upTeXのjfmとjvfの構造をあんまり把握していないのでそこから考えないといけないので保留です．pTeXがjisがベースであるところがupTeXではUTF8であろうというくらいの理解しかありません．

漢字の異体字でよく使うものへのショートカットについては，どういうインタフェースがよいのか悩んでいます．「梯子高」は「高」に関連付けるべきでしょうが，「斉」のようにたくさんの異体字があるものはどうするか．「斉1」，「斉2」のように連番をつけるのは避けたいですが，IVSにしたって連番みたいなもの，番号と字形に直感的にわかるつながりはありません．なるべく汎用的な・一般的なものを使いたいですがよくわかりません．
