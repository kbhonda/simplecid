...
多書体化の場合はpLaTeX2eでの和文の扱いの知識が必要ですが，使用したい書体に対してNFSSでどのような名前をつけているのか（`\DeclareFontShape`でどのように宣言しているのか）と，実際のフォントのファイル名が分かればいいだけです．それが分からない場合でも，`\sCID`を使った際に適切な設定がなされていないときにはそれなりのエラーがでるので，それを利用すればなんとかなるはずです．
