...
パディングに関しては，expl3のtoken listとstringの差であまりすっきりしていないところがあります．`\string`や`\the`で生成される「文字列」のカテゴリーコードが12であることに関わっているのですが，何かの操作の結果で現れる「文字列」がどちらなのかを間違えると一見理不尽な動きになるので注意が必要です．map系の関数を多用するとそのあたりで混乱しそうです．というか，しました．
